...
public class SshHostStep extends Step implements Serializable {
    ...
    public static class SshHostStepExecution extends SynchronousNonBlockingStepExecution<Void> {
        ...
        protected Void run() throws Exception {
            ...
            listener.getLogger().println("=== SSH Host: " + step.getHost() + " ===");
            listener.getLogger().println("Environment: " + environment.getName());
            listener.getLogger()
                    .println("User: " + environment.getUsername() + "@" + step.getHost() + ":" + environment.getPort());

            // Create SSH context
            SshContext sshContext = new SshContext(step.getHost(), environment, credentials, listener);

            // Test connection
            if (!sshContext.testConnection(step.getHost())) {
