...
/**
 * Management link that appears in "Manage Jenkins" section.
 * Provides a dedicated page for HelloWorld plugin configuration.
 */
@Extension
...
public class HelloWorldManagementLink extends ManagementLink {
    ...
    /**
     * URL segment for this management page.
     * Will be accessible at: /manage/helloworld
     */
    @Override
    ...
    /**
     * Icon name (from Jenkins icon set) or path to custom icon.
     * Jenkins provides many built-in icons like 'gear', 'folder', 'document', etc.
     */
    @Override
    ...
    /**
     * Category for grouping in Manage Jenkins page.
     * Common categories: "CONFIGURATION", "SECURITY", "STATUS", "TROUBLESHOOTING", "TOOLS"
     */
    @Override
    ...
    /**
     * Permission required to access this management page.
     * ADMINISTER = only Jenkins admins can access
     */
    @Override
    ...
    /**
     * Handle form submission from the management page.
     * This method is called when users submit the configuration form.
     */
    @POST
    ...
    /**
     * Test action to demonstrate AJAX calls from the UI.
     * This can be called via JavaScript: /manage/helloworld/testGreeting?message=test
     */
    public String doTestGreeting(@QueryParameter String message) {
        ...
        return "Test greeting: " + message + 
               (config.isIncludeTimestamp() ? " (at " + java.time.LocalDateTime.now() + ")" : "");
    }
}