        ...
        new Thread(() -> {
            try {
                ...
                if (computer == Jenkins.get().toComputer()) {
                    ...
                    LOGGER.warning("No channel available for node: " + nodeName + ", skipping container discovery");
                    return;
                }
    ...
    private void discoverAndAdoptContainers(String nodeName, hudson.Launcher launcher, TaskListener listener) {
        try {
            ...
            List<String> findCmd = Arrays.asList(
                ...
                "--filter", "label=" + ContainerManager.PLUGIN_LABEL,
                "--filter", "label=" + ContainerManager.NODE_LABEL_PREFIX + nodeFilter,
                ...
                if (!result.isEmpty()) {
                    ...
                    for (String containerId : containerIds) {
                        ...
                        List<String> inspectCmd = Arrays.asList(
                            ...
                            "--format", "{{index .Config.Labels \"io.jenkins.sharedcontainer.image\"}}"
                        );
                        ...
                        if (inspectExitCode == 0) {
                            String imageWithPrefix = inspectOutput.toString(StandardCharsets.UTF_8).trim();
                            if (!imageWithPrefix.isEmpty()) {
                                ...
                                if (!image.isEmpty()) {
                                    try {
                                        ...
                                        LOGGER.fine("Adopted managed container: " + getShortId(containerId) + " (image: " + image + ")");
                            ...
                            LOGGER.log(Level.WARNING, "Failed to inspect container {0} (exit code: {1})", 
                                     new Object[]{getShortId(containerId), inspectExitCode});
                        }
                    }
                } else {
