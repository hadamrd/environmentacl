...
public class SshAgentDiscovery extends ComputerListener {
     ...
     */
    private void categorizeAgentProcesses(String nodeFilter, Map<String, String> activePidToSocket, 
                                        List<String> zombiePids, hudson.Launcher launcher, TaskListener listener) {
        try {
     ...
     */
    private void cleanupOrphanedSockets(String nodeFilter, Map<String, String> activePidToSocket, 
                                      hudson.Launcher launcher, TaskListener listener) {
        try {
            List<String> findCmd = Arrays.asList(
                "/bin/sh", "-c",
                String.format("find %s -name 'agent-%s-*.sock' 2>/dev/null || true", 
     ...
     */
    private void handleActiveAgents(String nodeFilter, Map<String, String> activePidToSocket,
                                  hudson.Launcher launcher, TaskListener listener) {
        ...
        LOGGER.info("Adopted 1 SSH agent (PID " + adoptedPid + "), killed " + killedCount + " duplicates for node: " + nodeFilter);
    }
    ...
    private boolean forceKillAgent(String pid, hudson.Launcher launcher, TaskListener listener) {
        try {
            ...
            List<String> checkCmd = Arrays.asList(
                "/bin/sh", "-c",
                String.format("test ! -d /proc/%s || " +
                             "[ \"$(awk '{print $3}' /proc/%s/stat 2>/dev/null)\" = \"Z\" ]", pid, pid)
            );
            ...
            int exitCode = launcher.launch()
                ...
                .quiet(true)
                .start()
                .joinWithTimeout(3, TimeUnit.SECONDS, listener);
     ...
     */
    private void adoptSshAgent(String nodeName, String socketPath, String pid, hudson.Launcher launcher, TaskListener listener) {
        try {
